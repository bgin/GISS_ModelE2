...
![eml
     ...
     &                      nmassflux, mflx_count, do_mass_flux,
     &                      mflx_count1
#endif
!eml]
      IMPLICIT NONE
...
![eml
#ifdef CACHED_SUBDD
      ! Convert from mb to kg and multiply by DTLF to go from kg/s to kg
      if(do_mass_flux) then
         massfluxU1(:,:,:)=PU(:,:,:)*DTLF*mb2kg+massfluxU1(:,:,:)
         massfluxV1(:,:,:)=PV(:,:,:)*DTLF*mb2kg+massfluxV1(:,:,:)
         massfluxW1(:,:,:)=SD(:,:,1:LM-1)*DTLF*mb2kg+massfluxW1(:,:,:)
         mflx_count1=mflx_count1+1
!         massfluxU(:,:,:,nmassflux)=PU(:,:,:)*DTLF*mb2kg+
!     &                              massfluxU(:,:,:,nmassflux)
!         massfluxV(:,:,:,nmassflux)=PV(:,:,:)*DTLF*mb2kg+
!     &                              massfluxV(:,:,:,nmassflux)
!         massfluxW(:,:,1:LM-1,nmassflux)=SD(:,:,1:LM-1)*DTLF*mb2kg+
!     &                                   massfluxW(:,:,1:LM-1,nmassflux)
!         mflx_count=mflx_count+1
      endif
#endif
!eml]
