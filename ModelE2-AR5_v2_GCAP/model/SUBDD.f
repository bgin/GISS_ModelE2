...
![eml
      ! Logicals
      logical :: do_light = .FALSE.
      logical :: do_mass_flux = .FALSE. ! This is hard coded as of now
      logical :: do_moistq = .FALSE. ! changes to .TRUE. if listed in SUBDD names
      logical :: do_convect = .FALSE. ! changes to .TRUE. if listed in SUBDD
      logical :: do_MOLENGTH = .FALSE. ! changes to .TRUE. if listed in SUBDD
      logical :: do_USTAR = .FALSE. ! changes to .TRUE. if listed in SUBDD

      ! Lightning
      real*8,  allocatable :: gcaplightning(:,:),gcaplightning_c2g(:,:)
      integer, allocatable :: count_light(:,:), count_light_c2g(:,:)

      ! Mass fluxes
      integer :: nmassflux, mflx_count, nmflux_visits, mflx_count1
      real*8, allocatable :: massfluxU(:,:,:,:), massfluxV(:,:,:,:),
     &                       massfluxW(:,:,:,:), massfluxU1(:,:,:),
     &                       massfluxV1(:,:,:),massfluxW1(:,:,:)
     
      ! MOISTQ
      real*8, allocatable :: moistq(:,:,:)
      integer,allocatable :: count_moistq(:,:,:)

      ! Convection
      real*8, allocatable :: up1(:,:,:) ! less-entraining upd. 
      integer,allocatable :: count_up1(:,:,:)
      real*8, allocatable :: up2(:,:,:) ! more-entraining upd.
      integer,allocatable :: count_up2(:,:,:)
      real*8, allocatable :: eu1(:,:,:) ! ent.  into less-entraining
      integer,allocatable :: count_eu1(:,:,:)
      real*8, allocatable :: eu2(:,:,:) ! ent. into more-entraining
      integer,allocatable :: count_eu2(:,:,:)
      real*8, allocatable :: dd1(:,:,:) ! downdraft of less-ent. 
      integer,allocatable :: count_dd1(:,:,:)
      real*8, allocatable :: dd2(:,:,:) ! downdraft of more ent.
      integer,allocatable :: count_dd2(:,:,:)
      real*8, allocatable :: ed1(:,:,:) ! entrainement to downdraft
      integer,allocatable :: count_ed1(:,:,:)
      real*8, allocatable :: ed2(:,:,:) ! ent. to more ent. downdraft
      integer,allocatable :: count_ed2(:,:,:) 
      real*8, allocatable :: ou2(:,:,:) ! outflow from more ent. plume
      integer,allocatable :: count_ou2(:,:,:)
      real*8, allocatable :: ou1(:,:,:) ! outflow from less ent. plume
      integer,allocatable :: count_ou1(:,:,:)
      real*8, allocatable :: od2(:,:,:) ! outflow from more ent. dd
      integer,allocatable :: count_od2(:,:,:)
      real*8, allocatable :: od1(:,:,:) ! outflow from less ent. dd
      integer,allocatable :: count_od1(:,:,:)

      ! Misc.
      real*8, allocatable :: savealb(:,:) ! Albedo
      real*8, allocatable :: MOLENGTH(:,:), MO_PTYPE(:,:) ! MO Length
      real*8, allocatable :: USTAR(:,:), US_PTYPE(:,:) ! USTAR
!eml]
...
![eml
      use subdd_mod, only : do_MOLENGTH,do_USTAR,do_mass_flux,do_convect
     &                      ,do_moistq,do_light
...
![eml
            if(namedd(k).eq.'MOLENGTH') do_MOLENGTH=.TRUE.
            if(namedd(k).eq.'USTAR') do_USTAR=.TRUE.
            if(namedd(k).eq.'MFLUXU'.or.
     &         namedd(k).eq.'MFLUXV'.or.
     &         namedd(k).eq.'MFLUXW') do_mass_flux=.TRUE.
            if(namedd(k).eq.'C_UP1'.or.namedd(k).eq.'C_UP2'.or.
     &        namedd(k).eq.'C_DD1'.or.namedd(k).eq.'C_DD2'.or.
     &        namedd(k).eq.'C_EU1'.or.namedd(k).eq.'C_EU2'.or.
     &        namedd(k).eq.'C_OU1'.or.namedd(k).eq.'C_OU2'.or.
     &        namedd(k).eq.'C_ED1'.or.namedd(k).eq.'C_ED2'.or.
     &        namedd(k).eq.'C_OD1'.or.namedd(k).eq.'C_OD2')
     &           do_convect=.TRUE.
            if(namedd(k).eq.'MOISTQ') do_moistq=.TRUE.
            if(namedd(k).eq.'lightn' .or. namedd(k).eq.'lightn_c2g') 
     &        do_light=.TRUE.
!eml]
          else
...
![eml
      if(am_i_root()) then
         print*, 'Do Monin-Obukhov Length: ', do_MOLENGTH
         print*, 'Do USTAR: ', do_USTAR
         print*, 'Do Mass Flux: ', do_mass_flux
         print*, 'Do MOISTQ: ', do_moistq
         print*, 'Do Convection: ', do_convect
      endif
!eml]
      do k=1,kdd
...
![eml Add some used variables and routines
      use domain_decomp_atm, only : grid, get
      use subdd_mod, only : savealb, MOLENGTH, MO_PTYPE,do_MOLENGTH
      use subdd_mod, only : USTAR, US_PTYPE, do_USTAR
      use subdd_mod, only : gcaplightning,gcaplightning_c2g,
     &                      count_light, count_light_c2g, do_light
!eml]
      implicit none
      integer :: nmax,decl_count
![eml Add loop variables
...
![eml Change tsavg to TS and unit from C to K
      arr(next()) = info_type_(
     &  sname = 'TS',!'tsavg',
...
c
     ...
     &     )
c
![eml Change from sname of prec to PREACC
      arr(next()) = info_type_(
     &  sname = 'PREACC',
     &  lname = 'PRECIPITATION',
     &  units = 'mm/day',
...
c
     ...
     &     )
c
![eml change us and vs to USS and VSS
      arr(next()) = info_type_(
     &  sname = 'USS',!'us',
...
![eml Add the following
     ...
     &  units = 'flashes/m2/s' )
      arr(next()) = info_type_(
     &  sname = 'lightn_c2g',
     &  lname = 'CLOUD-TO-GROUND LIGHTNING',
     &  units = 'flashes/m2/s' )
      if(.not.allocated(gcaplightning).and.do_light) then
        call get(grid,i_strt=i_0,i_stop=i_1,
     &                j_strt=j_0,j_stop=j_1)
        allocate(gcaplightning(i_0:i_1,j_0:j_1))
        allocate(gcaplightning_c2g(i_0:i_1,j_0:j_1))
        allocate(count_light(i_0:i_1,j_0:j_1))
        allocate(count_light_c2g(i_0:i_1,j_0:j_1))
        count_light_c2g(i_0:i_1,j_0:j_1)=0.;
        count_light(i_0:i_1,j_0:j_1)=0.
        gcaplightning(i_0:i_1,j_0:j_1)=0.
        gcaplightning_c2g(i_0:i_1,j_0:j_1)=0.
      endif
      arr(next()) = info_type_(
     &  sname = 'ALBEDO', 
     &  lname = 'ALBEDO',
     &  units = 'fraction' )
      if(.not.allocated(savealb)) then
         call get(grid,i_strt=i_0,i_stop=i_1,
     &                 j_strt=j_0,j_stop=j_1)
         allocate(savealb(i_0:i_1,j_0:j_1))
         savealb(i_0:i_1,j_0:j_1)=1. ! set to 1 so that no data (salb=0) means 
                                     ! 1 - 1 = 0.
      endif
      arr(next()) = info_type_(
     &  sname = 'MOLENGTH', 
     &  lname = 'M.O. LENGTH',
     &  units = 'm' )
      if(do_MOLENGTH.and..not.allocated(MOLENGTH)) then
         call get(grid,i_strt=i_0,i_stop=i_1,
     &                 j_strt=j_0,j_stop=j_1)
         allocate(MOLENGTH(i_0:i_1,j_0:j_1), MO_PTYPE(i_0:i_1,j_0:j_1))
         MOLENGTH(i_0:i_1,j_0:j_1)=0.; MO_PTYPE(i_0:i_1,j_0:j_1)=0.
      endif
      arr(next()) = info_type_(
     &  sname = 'USTAR', 
     &  lname = 'FRICTION WIND SPEED',
     &  units = 'm/s' )
      if(do_USTAR.and..not.allocated(USTAR)) then
         call get(grid,i_strt=i_0,i_stop=i_1,
     &                 j_strt=j_0,j_stop=j_1)
         allocate(USTAR(i_0:i_1,j_0:j_1), US_PTYPE(i_0:i_1,j_0:j_1))
         USTAR(i_0:i_1,j_0:j_1)=0.; US_PTYPE(i_0:i_1,j_0:j_1)=0.
      endif
!eml]
c
      arr(next()) = info_type_(
...
c
     ...
     &     )
c
![eml Change sname from mcp to PRECON
      arr(next()) = info_type_(
     &  sname = 'PRECON',
...
c
     ...
     &     )
c
![eml Add OICE, which is FOICE + FLKICE
      arr(next()) = info_type_(
     &  sname = 'OICE',
     &  lname = 'Ocean + Lake Ice Cover',
     &  units = 'fraction'
     &     )
      arr(next()) = info_type_(
     &  sname = 'SNICE',
     &  lname = 'Snow and Ice Cover',
     &  units = 'fraction'
     &     )
!eml]
      arr(next()) = info_type_(
...
![eml Change swd to RADSWG
     ...
     &     )
!eml]
![eml Add PARDR and PARDF
      arr(next()) = info_type_(
     &  sname = 'PARDR',
     &  lname = 'DIRECT PAR at SURFACE',
     &  units = 'W/m^2',
     &  sched = sched_rad
     &     )      
      arr(next()) = info_type_(
     &  sname = 'PARDF',
     &  lname = 'DIFFUSE PAR at SURFACE',
     &  units = 'W/m^2',
     &  sched = sched_rad
     &     )      
!eml]
...
c
     ...
     &     )
c
![eml Change from snowd to SNOW and from mm to m
      arr(next()) = info_type_(
     &  sname = 'SNOW',!'snowd',
     &  lname = 'Snow depth',
     &  units = 'm water equiv',
...
! info_type_ is a homemade structure constructor for older compilers
![eml
      use subdd_mod, only: massfluxU, massfluxV,massfluxW,nmflux_visits,
     &                      do_mass_flux, nmassflux, mflx_count
      use subdd_mod, only : mflx_count1,massfluxU1,massfluxV1,massfluxW1
      use subdd_mod, only : up1,count_up1,up2,count_up2
      use subdd_mod, only : eu1,count_eu1,eu2,count_eu2
      use subdd_mod, only : ou1,count_ou1,ou2,count_ou2
      use subdd_mod, only : dd1,count_dd1,dd2,count_dd2
      use subdd_mod, only : ed1,count_ed1,ed2,count_ed2
      use subdd_mod, only : od1,count_od1,od2,count_od2
      use subdd_mod, only : moistq, count_moistq
...
!eml]
     &     )
![eml Add OPTDEPTH and 3d cloud fraction
      arr(next()) = info_type_(
     &  sname = 'OPTDEPTH',
     &  lname = 'CLOUD OPTICAL DEPTH',
     &  units = '-',
     &  sched = sched_rad)
      arr(next()) = info_type_(
     &  sname = 'CLDF',
...
![eml Add mass fluxes
      arr(next()) = info_type_(
     &  sname = 'MFLUXU',
     &  lname = 'EASTWARD MASS FLUX',
     &  units = 'kg per DTLF timestep')
      arr(next()) = info_type_(
     &  sname = 'MFLUXV',
     &  lname = 'NORTHWARD MASS FLUX',
     &  units = 'kg per DTLF timestep')
      arr(next()) = info_type_(
     &  sname = 'MFLUXW',
...
!         if(.not.allocated(massfluxU)) then
!            call get(grid,i_strt_halo=i_0,i_stop_halo=i_1,
!     &               j_strt_halo=j_0,j_stop_halo=j_1)
!            allocate(massfluxU(i_0:i_1,j_0:j_1,lm,4),
!     &               massfluxV(i_0:i_1,j_0:j_1,lm,4),
!     &               massfluxW(i_0:i_1,j_0:j_1,lm-1,4))
!            nmassflux=1; mflx_count=0; nmflux_visits=0
!         endif
         if (.not.allocated(massfluxU1)) then
            call get(grid,i_strt_halo=i_0,i_stop_halo=i_1,
     &               j_strt_halo=j_0,j_stop_halo=j_1)
            mflx_count1=0.
            allocate(massfluxU1(i_0:i_1,j_0:j_1,lm),
     &               massfluxV1(i_0:i_1,j_0:j_1,lm),
     &               massfluxW1(i_0:i_1,j_0:j_1,lm-1))
            massfluxU1=0.;massfluxV1=0.;massfluxW1=0.
         endif
      endif
! Add convection
      arr(next()) = info_type_(
     &  sname ='C_UP1',lname='non-entraining updraft',units='Pa/s')
      arr(next()) = info_type_(
     &  sname ='C_UP2',lname='entraining updraft',units='Pa/s')
      arr(next()) = info_type_(
     &  sname ='C_DD1',lname='non-entraining downdraft',units='Pa/s')
      arr(next()) = info_type_(
     &  sname ='C_DD2',lname='entraining downdraft',units='Pa/s')
      arr(next()) = info_type_(
     &  sname ='C_EU1',lname='entrainment to ent updraft',units='Pa/s')
      arr(next()) = info_type_(
     &  sname ='C_EU2',lname='entrainment to non-ent upd',units='Pa/s')
      arr(next()) = info_type_(
     & sname ='C_ED1',lname='entrainment to ent downdraft',units='Pa/s')
      arr(next()) = info_type_(
     & sname ='C_ED2',lname='entrainment to non-ent dd',units='Pa/s')
      arr(next()) = info_type_(
     &sname ='C_OU1',lname='outflow of non-entraining upd',units='Pa/s')
      arr(next()) = info_type_(
     &sname ='C_OU2',lname='outflow of entraining updraft',units='Pa/s')
      arr(next()) = info_type_(
     & sname ='C_OD1',lname='outflow of non-entraining dd',units='Pa/s')
      arr(next()) = info_type_(
     & sname ='C_OD2',lname='outflow of entraining ddraft',units='Pa/s')
      if(.not.allocated(up1)) then
         call get(grid,i_strt=i_0,i_stop=i_1,
     &                j_strt=j_0,j_stop=j_1)
         allocate(up1(i_0:i_1,j_0:j_1,lm),up2(i_0:i_1,j_0:j_1,lm),
     &            dd1(i_0:i_1,j_0:j_1,lm),dd2(i_0:i_1,j_0:j_1,lm),
     &            eu1(i_0:i_1,j_0:j_1,lm),eu2(i_0:i_1,j_0:j_1,lm),
     &            od1(i_0:i_1,j_0:j_1,lm),od2(i_0:i_1,j_0:j_1,lm),
     &            ed1(i_0:i_1,j_0:j_1,lm),ed2(i_0:i_1,j_0:j_1,lm),
     &            ou1(i_0:i_1,j_0:j_1,lm),ou2(i_0:i_1,j_0:j_1,lm),
     &      count_up1(i_0:i_1,j_0:j_1,lm),count_up2(i_0:i_1,j_0:j_1,lm),
     &      count_dd1(i_0:i_1,j_0:j_1,lm),count_dd2(i_0:i_1,j_0:j_1,lm),
     &      count_eu1(i_0:i_1,j_0:j_1,lm),count_eu2(i_0:i_1,j_0:j_1,lm),
     &      count_ed1(i_0:i_1,j_0:j_1,lm),count_ed2(i_0:i_1,j_0:j_1,lm),
     &      count_ou1(i_0:i_1,j_0:j_1,lm),count_ou2(i_0:i_1,j_0:j_1,lm),
     &      count_od1(i_0:i_1,j_0:j_1,lm),count_od2(i_0:i_1,j_0:j_1,lm)
     &    )
         up1(i_0:i_1,j_0:j_1,:)=0.; up2(i_0:i_1,j_0:j_1,:)=0.
         dd1(i_0:i_1,j_0:j_1,:)=0.; dd2(i_0:i_1,j_0:j_1,:)=0.
         eu1(i_0:i_1,j_0:j_1,:)=0.; eu2(i_0:i_1,j_0:j_1,:)=0.
         ed1(i_0:i_1,j_0:j_1,:)=0.; ed2(i_0:i_1,j_0:j_1,:)=0.
         ou1(i_0:i_1,j_0:j_1,:)=0.; ou2(i_0:i_1,j_0:j_1,:)=0.
         od1(i_0:i_1,j_0:j_1,:)=0.; od2(i_0:i_1,j_0:j_1,:)=0.
         count_up1(i_0:i_1,j_0:j_1,:)=0.;count_up2(i_0:i_1,j_0:j_1,:)=0.
         count_dd1(i_0:i_1,j_0:j_1,:)=0.;count_dd2(i_0:i_1,j_0:j_1,:)=0.
         count_eu1(i_0:i_1,j_0:j_1,:)=0.;count_eu2(i_0:i_1,j_0:j_1,:)=0.
         count_ed1(i_0:i_1,j_0:j_1,:)=0.;count_ed2(i_0:i_1,j_0:j_1,:)=0.
         count_ou1(i_0:i_1,j_0:j_1,:)=0.;count_ou2(i_0:i_1,j_0:j_1,:)=0.
         count_od1(i_0:i_1,j_0:j_1,:)=0.;count_od2(i_0:i_1,j_0:j_1,:)=0.
      endif
      arr(next()) = info_type_(
     &  sname = 'MOISTQ',
     &  lname = 'water tendency',
     &  units = 'gH2O/kg air/day'
     &     )
      if(.not. allocated(moistq)) then
        call get(grid,i_strt=i_0,i_stop=i_1,
     &                j_strt=j_0,j_stop=j_1)
        allocate(moistq(i_0:i_1,j_0:j_1,lm))
        allocate(count_moistq(i_0:i_1,j_0:j_1,lm))
        moistq(i_0:i_1,j_0:j_1,:)=0.
        count_moistq(i_0:i_1,j_0:j_1,:)=0
      endif
...
![eml Change to U
     ...
     &  sname = 'UA',!'u',
!eml]
     &  lname = 'east-west velocity (A-GRID)',
     &  units = 'm/s'
     &     )
c
![eml Change to V
     ...
     &  sname = 'VA',!'v',
!eml]
     &  lname = 'north-south velocity (A-GRID)',
     &  units = 'm/s'
     &     )
c
![eml change to UB and VB eml]
      arr(next()) = info_type_(
     &  sname = 'UB',
...
![eml
      ! For albedo
      use subdd_mod,only : savealb
      use RAD_COM, only  : srdn, alb
      ! For MO Length
      use subdd_mod, only : MOLENGTH, MO_PTYPE, USTAR, US_PTYPE
      use MODEL_COM, only : NISURF
      ! For Mass fluxes
      use subdd_mod, only : do_mass_flux,massfluxU1,mflx_count1,
     &                      massfluxV1,massfluxW1
      ! For convection
      use subdd_mod, only : up1, count_up1, up2, count_up2
      use subdd_mod, only : dd1, count_dd1, dd2, count_dd2
...
![eml
!      if(do_mass_flux) call accum_mass_flux
!eml]
     ...
     &               J_STRT=J_0,        J_STOP=J_1)
      ...
      select case (subdd%name(k))
C
![eml Change p_surf to PS and slp to SLP
      case ('PS')!'p_surf')
        sddarr2d = p(:,:) + ptop
...
![eml Add the following
      case ('ALBEDO')
         do j=j_0,j_1; do i=i_0,imaxj(j)
            if(srdn(i,j).gt.0) savealb(i,j)=alb(i,j,1)
            sddarr2d(i,j)=1.d0 - savealb(i,j)
         enddo; enddo
         call inc_subdd(subdd,k,sddarr2d)
      case ('MOLENGTH')
         do j=j_0,j_1; do i=i_0,imaxj(j)
            sddarr2d(i,j)=MOLENGTH(i,j)/(MO_PTYPE(i,j)+1e-20)
            MOLENGTH(i,j)=0.; MO_PTYPE(i,j)=0.
         enddo; enddo
         call inc_subdd(subdd,k,sddarr2d)
      case ('USTAR')
         do j=j_0,j_1; do i=i_0,imaxj(j)
            sddarr2d(i,j)=USTAR(i,j)/(US_PTYPE(i,j)+1e-20)
            USTAR(i,j)=0.; US_PTYPE(i,j)=0.
         enddo; enddo
         call inc_subdd(subdd,k,sddarr2d)
!eml]
      end select
...
!eml]
        call inc_subdd(subdd,k,valij,jdim=3)
#ifndef CUBED_SPHERE
![eml change from ub to UB and vb to VB
...
![eml
      case ('lightn')
         do j=j_0,j_1; do i=i_0,imaxj(j)
            sddarr2d(i,j)=gcaplightning(i,j)/(count_light(i,j)+1e-30)
            count_light(i,j)=0; gcaplightning(i,j)=0.
         enddo; enddo
         call inc_subdd(subdd,k,sddarr2d)
      case ('lightn_c2g')
         do j=j_0,j_1; do i=i_0,imaxj(j)
            sddarr2d(i,j)=gcaplightning_c2g(i,j)/
     &                         (count_light_c2g(i,j)+1e-30)
            count_light_c2g(i,j)=0; gcaplightning_c2g(i,j)=0.
         enddo; enddo
         call inc_subdd(subdd,k,sddarr2d)
      case ('MFLUXU')
        sddarr(:,:,:) = massfluxU1/mflx_count1 ! This gives avg. per DTLF
        call inc_subdd(subdd,k,sddarr)
        massfluxU1(:,:,:)=0.
      case ('MFLUXV')
        sddarr(:,:,:) = massfluxV1/mflx_count1
        call inc_subdd(subdd,k,sddarr)
        massfluxV1(:,:,:)=0.
      case ('MFLUXW')
        sddarr(:,:,:)=0.!just make sure LM layer is 0
        sddarr(:,:,1:LM-1) = massfluxW1/mflx_count1
        call inc_subdd(subdd,k,sddarr)
        massfluxW1(:,:,:)=0.
      ! These convective fluxes are sum of the mass flux (mb) over thr
      ! dtsrc timestep between calls to accum_subdd
      case ('C_UP1')
        do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,:)=100.*up1(i,j,:)/dtsrc
        up1(i,j,:)=0;
        enddo;enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_UP2')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*up2(i,j,l)/dtsrc
           up2(i,j,l)=0;
           !count_up2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_EU1')
        do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,:)=100.*eu1(i,j,:)/dtsrc
        eu1(i,j,:)=0;
        !count_eu1(i,j,:)=0
        enddo;enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_EU2')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*eu2(i,j,l)/dtsrc
           eu2(i,j,l)=0;
        !   count_eu2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_ED1')
        do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,:)=100.*ed1(i,j,:)/dtsrc
        ed1(i,j,:)=0;
        !count_ed1(i,j,:)=0
        enddo;enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_ED2')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*ed2(i,j,l)/dtsrc
           ed2(i,j,l)=0;
         !  count_ed2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_DD1') !defined to be negative
        do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,:)=-100.*dd1(i,j,:)/dtsrc
        dd1(i,j,:)=0;
        !count_dd1(i,j,:)=0
        enddo;enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_DD2') !defined to be negative
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=-100.*dd2(i,j,l)/dtsrc
           dd2(i,j,l)=0;
        !   count_dd2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_OU1')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*ou1(i,j,l)/dtsrc
           ou1(i,j,l)=0;
        !   count_ou1(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_OU2')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*ou2(i,j,l)/dtsrc
           ou2(i,j,l)=0;
        !   count_ou2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_OD1')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*od1(i,j,l)/dtsrc
           od1(i,j,l)=0;
        !   count_od1(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('C_OD2')
        do l=1,lm; do j=j_0,j_1; do i=i_0,imaxj(j)
           sddarr(i,j,l)=100.*od2(i,j,l)/dtsrc
           od2(i,j,l)=0;
        !   count_od2(i_0:imaxj(j),j_0:j_1,:)=0
        enddo; enddo; enddo
        call inc_subdd(subdd,k,sddarr)
      case ('MOISTQ')
        do j=j_0,j_1; do i=i_0,imaxj(j); do l=1,lm
           sddarr(i,j,l)=moistq(i,j,l)/(real(count_moistq(i,j,l))+1e-30)
           moistq(i,j,l)=0.; count_moistq(i,j,l)=0
        enddo; enddo; enddo
        call inc_subdd(subdd, k, sddarr)
!eml]
      end select
...
![eml
!        if(am_i_root()) print*,dimstr
!        if(am_i_root()) print*,trim(subdd_groups(k)%strdimlen)
!eml]
        if(allocated(subdd_groups(k)%v4d)) then
...
#endif
![eml
      use subdd_mod, only : do_mass_flux
!eml]
      implicit none
!@var fname base name of file to be read or written
      ...
      integer :: k
![eml
!      if(do_mass_flux) call write_mass_flux_file(fname)
!eml]
c
...
![eml Accumulate routine for sub DTsrc quantities
      subroutine accumulate_gcap(val,variable,i,j,l,val1)
            ! These quantities are updated every run through their physics routines, 
      ! so they are frequently updated. This creates a better average.
      use domain_decomp_atm, only : grid, get
      use domain_decomp_atm, only : am_i_root
      use resolution, only : lm
      use subdd_mod, only : up1, count_up1, up2, count_up2
      ...
      use subdd_mod,only: MOLENGTH, MO_PTYPE, USTAR, US_PTYPE
      use subdd_mod,only: gcaplightning,gcaplightning_c2g,
     &                    count_light, count_light_c2g
      implicit none
      integer :: i_0,i_1,j_0,j_1,i,j,l,j_0s,j_1s,j_0stg,j_1stg
      real*8,intent(in) :: val
      real*8,intent(in), optional :: val1
     ...
     &              j_strt_skp=j_0s,j_stop_skp=j_1s)
      ...
      case ('wtd')
         moistq(i,j,l)=val+moistq(i,j,l)
         count_moistq(i,j,l)=1+count_moistq(i,j,l)
      case ('lit')
         gcaplightning(i,j)=gcaplightning(i,j)+val
         count_light(i,j)=count_light(i,j)+1
      case ('c2g')
         gcaplightning_c2g(i,j)=gcaplightning_c2g(i,j)+val
         count_light_c2g(i,j)=count_light_c2g(i,j)+1
      case ('MOL')
         if(.not.present(val1)) STOP 413
         MOLENGTH(i,j) = val*val1+MOLENGTH(i,j)
         MO_PTYPE(i,j) = val1    +MO_PTYPE(i,j)
      case ('UST')
         if(.not.present(val1)) STOP 413
         USTAR(i,j) = val*val1+USTAR(i,j)
         US_PTYPE(i,j) = val1 +US_PTYPE(i,j)
!      case ('sol')
!         soil_gcap(i,j)=soil_gcap(i,j)+val
!         soil_count(i,j)=soil_count(i,j)+1
      case ('up1')
         up1(i,j,l)=val+up1(i,j,l);count_up1(i,j,l)=count_up1(i,j,l)+1
      case ('up2')
         up2(i,j,l)=val+up2(i,j,l);count_up2(i,j,l)=count_up2(i,j,l)+1
      case ('dd1')
         dd1(i,j,l)=val+dd1(i,j,l);count_dd1(i,j,l)=count_dd1(i,j,l)+1
      case ('dd2')
         dd2(i,j,l)=val+dd2(i,j,l);count_dd2(i,j,l)=count_dd2(i,j,l)+1
      case ('eu1')
         eu1(i,j,l)=val+eu1(i,j,l);count_eu1(i,j,l)=count_eu1(i,j,l)+1
      case ('eu2')
         eu2(i,j,l)=val+eu2(i,j,l);count_eu2(i,j,l)=count_eu2(i,j,l)+1
      case ('ed1')
         ed1(i,j,l)=val+ed1(i,j,l);count_ed1(i,j,l)=count_ed1(i,j,l)+1
      case ('ed2')
         ed2(i,j,l)=val+ed2(i,j,l);count_ed2(i,j,l)=count_ed2(i,j,l)+1
      case ('ou1')
         ou1(i,j,l)=val+ou1(i,j,l);count_ou1(i,j,l)=count_ou1(i,j,l)+1
      case ('ou2')
         ou2(i,j,l)=val+ou2(i,j,l);count_ou2(i,j,l)=count_ou2(i,j,l)+1
      case ('od1')
         od1(i,j,l)=val+od1(i,j,l);count_od1(i,j,l)=count_od1(i,j,l)+1
      case ('od2')
         od2(i,j,l)=val+od2(i,j,l);count_od2(i,j,l)=count_od2(i,j,l)+1
      case default
         print*,trim(variable)
         stop 460
      end select
...
!      subroutine accum_mass_flux
!      use domain_decomp_atm, only : grid, get, am_i_root
!      use subdd_mod, only : massfluxU, massfluxV, massfluxW, 
!     &                      nmassflux, mflx_count,nmflux_visits
!      implicit none
!      integer :: J_0, J_1, I_0, I_1
!      
!      nmflux_visits=nmflux_visits+1
!      if(nmflux_visits.eq.12) then
!         CALL GET(grid,i_strt=i_0,i_stop=i_1,j_strt=j_0,j_stop=j_1)
!         massfluxU(i_0:i_1,j_0:j_1,:,nmassflux)=
!     &     massfluxU(i_0:i_1,j_0:j_1,:,nmassflux)/real(mflx_count)
!         massfluxV(i_0:i_1,j_0:j_1,:,nmassflux)=
!     &     massfluxV(i_0:i_1,j_0:j_1,:,nmassflux)/real(mflx_count)
!         massfluxV(i_0:i_1,j_0:j_1,:,nmassflux)=
!     &     massfluxV(i_0:i_1,j_0:j_1,:,nmassflux)/real(mflx_count)
!         nmassflux=nmassflux+1 
!         !if(am_i_root()) print*,'Number of flux accruals: ',mflx_count
!         if(nmassflux.gt.4) nmassflux=1
!         mflx_count=0; nmflux_visits=0;
!      endif
!      return
!      end subroutine accum_mass_flux
  
!      subroutine write_mass_flux_file(fname)
!      use model_com, only : xlabel
!      use domain_decomp_atm, only : grid
!      use pario, only : par_open, defvar, par_enddef, par_close, 
!     &                  write_attr, write_dist_data!,write_data
!      use subdd_mod, only : massfluxU, massfluxV, massfluxW, 
!     &                  nmassflux, mflx_count
!      use resolution, only : lm
!      implicit none
!      character(len=*) :: fname
!      character(len=64) :: dimstrU,dimstrV,dimstrW
!      integer :: massflux_id
!      if(nmassflux.ne.1) stop 4120
!      massflux_id=par_open(grid,trim(fname)//'.mflx.nc','create')
!      call write_attr(grid,massflux_id,'global','xlabel',xlabel)
!      dimstrU='mfluxU(dist_im,dist_jm,lm,4)'
!      call defvar(grid,massflux_id,massfluxU,trim(dimstrU),
!     &            r4_on_disk=.true.)
!      dimstrV='mfluxV(dist_im,dist_jm,lm,4)'
!      call defvar(grid,massflux_id,massfluxV,trim(dimstrV), 
!     &            r4_on_disk=.true.)
!      dimstrW='mfluxW(dist_im,dist_jm,lm-1,4)'
!      call defvar(grid,massflux_id,massfluxW,trim(dimstrW),
!     &            r4_on_disk=.true.)
!     call par_enddef(grid,massflux_id)
!     call write_dist_data(grid,massflux_id,'mfluxU',massfluxU)
!      call write_dist_data(grid,massflux_id,'mfluxV',massfluxV)
!      call write_dist_data(grid,massflux_id,'mfluxW',massfluxW)
!!      call write_data(grid,massflux_id,'massfluxU',massfluxU)
!!      call write_data(grid,massflux_id,'massfluxV',massfluxV)
!!      call write_data(grid,massflux_id,'massfluxW',massfluxW)
!      call par_close(grid,massflux_id)
!      massfluxU(:,:,:,:)=0.; massfluxV(:,:,:,:)=0.;
!      massfluxW(:,:,:,:)=0.; nmassflux=1; mflx_count=0
!      return
!      end subroutine write_mass_flux_file
!eml]
