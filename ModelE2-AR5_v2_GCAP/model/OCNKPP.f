...
c     USE ODIAG, only : oijl=>oijl_loc,oij=>oij_loc,oijmm
c    *     ,ij_hbl,ij_hblmax,ij_bo,ij_bosol,ij_ustar,ijl_kvm,ijl_kvg
c    *     ,ijl_wgfl,ijl_wsfl,ol,l_rho,l_temp,l_salt  !ij_ogeoz
c    *     ,ij_mld,ij_mldmax
...
#endif
c     USE OFLUXES, only : oRSI, oSOLARw,oSOLARi, oDMUA,oDMVA,oDMUI,oDMVI
      USE OFLUXES, only : oRSI, oSOLAR, oDMUA,oDMVA, oDMUI,oDMVI
      USE SW2OCEAN, only : fsr,lsrpd
c     Use DOMAIN_DECOMP_1d, Only: GETDomainBounds, HALO_UPDATE, NORTH,
     ...
     *      DTP4UV(LMO,IM+2),DTP4G(LMO),DTP4S(LMO),DTP4DUM(LMO)
#ifdef OCN_GISS_SM
...
#endif
      real*8 ptdd,ptdm,ptd(lmo),mld,wg0,ws0
...
c     call getDomainBounds(grid,j_strt_halo=j_0h)
      call get(grid,j_strt_halo=j_0h)

      DTP4G=0.d0; DTP4S=0.d0; DTP4UV=0.d0; DTP4DUM=0.d0
...
#ifdef ENHANCED_DEEP_MIXING
      KVBLdeep = 1.325d-4 ! abyssal asymptote of B-L profile (m2/s)
      KVBLz0 =  ! term to use in B-L profile to match fkph at z=0
     &  (fkph*1d-4 -KVBLdeep*brylew_shape(0d0))/(1d0-brylew_shape(0d0))
      do l=1,lmo-1
        ! option 1: B-L profile
c        kvextra(l) = .75d-5 +
c     &     (1.325d-4-.75d-5)*(atan((ze(l)-2000d0)/200d0)/pi + 0.5d0)
        kvextra(l) = KVBLz0 + (KVBLdeep-KVBLz0)*brylew_shape(ze(l))
...
C**** DMUA/I now defined over whole box, not just surface type
      ...
      VISTR = VISTR/IM
      U2rho = SQRT(
c    *     (oDMUA(1,JM) + UISTR)**2+
c    *     (oDMVA(1,JM) + VISTR)**2)*BYDTS
...
c     DELTASR= (oSOLARw(1,JM)*(1d0-oRSI(1,JM))+oSOLARi(1,JM)
...
C**** DMUA/I now defined over whole box, not just surface type
      ...
      IF (oRSI(I,J).gt.0) THEN
        ...
        VISTR = VISTR*ANSTR
      END IF
c     U2rho = SQRT((oDMUA(I,J) + UISTR)**2 +
c    *             (oDMVA(I,J) + VISTR)**2)*BYDTS
...
c      DELTASR = (oSOLARw(I,J)*(1d0-oRSI(I,J))+oSOLARi(I,J)*oRSI(I,J))
...
C**** betaDS   = mean sbeta  * delta(salt)  at interfaces (kg/m3)
     ...
     *     AKVM,AKVS,AKVG,GHAT,HBL,KBL)
      akvc=akvs
c     if(am_i_root()) then
c     write(21,'(i4,9e14.5)') KBL,HBL
c     do l=1,lmij-1
c     write(21,'(i4,9e14.5)') l,AKVM(l),AKVS(l),AKVG(l),GHAT(l)
c     end do
c     end if
#else
...
C****                            ghat (s/m^2) => (s m^4/kg^2)
      wg0 = DELTAE*DXYPO(J)
      ws0 = (DELTAS-S0ML0(1)*BYMML(1)*DELTAM)*DXYPO(J)
      DO L=1,LMIJ-1
#ifdef ENHANCED_DEEP_MIXING
...
c        GHATG(L) = AKVG(L)*GHAT(L)*wg0
c        GHATS(L) = AKVS(L)*GHAT(L)*ws0
...
c    &                      /(DELTAS-S0ML0(1)*BYMML(1)*DELTAM+1d-30)
      ...
      DTP4G=0.; DTP4S=0.; DTP4UV=0.; DTP4DUM=0.
#ifdef TRACERS_OCEAN
      DTP4TR=0.
#endif
#ifdef OCN_GISS_SM
...
C**** Momentum
      DO K=1,KMUV
        IF(LMUV(K).GT.1) THEN
          CALL OVDIFF(UL(1,K),AKVM(1),GHATM(1,K),DTP4UV(1,K)
...
c     if(am_i_root()) then
c     do l=1,lmij-1
c     write(22,'(i4,9e14.5)') l,UL(l,1),UL(l,2),UL(l,3),UL(L,4)
c     write(23,'(i4,9e14.5)') l,G0ML(l),S0ML(l)
c     end do
c     end if
C**** D-grid velocities
      If (.not.QPOLE)  Then
        DO K=1,KMUV
          IF(LMUV(K).GT.1) THEN
            CALL OVDIFF(ULD(1,K),AKVM(1),GHATM(1,K),DTP4UV(1,K)
...
c      OIJmm(I,J,IJ_HBLmax) = max(OIJmm(I,J,IJ_HBLmax), HBL)
c      OIJmm(I,J,ij_mldmax) = max(OIJmm(I,J,IJ_mldmax), mld)
...
C****
      ...
      do i=i1yzm(ib,j,1),i2yzm(ib,j,1)
        ...
        Call OVDIFFS (GXML(1),AKVG(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,GXML(1),FLDUM)
        Call OVDIFFS (GYML(1),AKVG(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,GYML(1),FLDUM)
        Call OVDIFFS (SXML(1),AKVS(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,SXML(1),FLDUM)
        Call OVDIFFS (SYML(1),AKVS(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,SYML(1),FLDUM)
        ...
        Call OVDIFFS (GXXML(1),AKVG(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,GXXML(1),FLDUM)
        Call OVDIFFS (GYYML(1),AKVG(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,GYYML(1),FLDUM)
        Call OVDIFFS (GXYML(1),AKVG(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,GXYML(1),FLDUM)
        Call OVDIFFS (SXXML(1),AKVS(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,SXXML(1),FLDUM)
        Call OVDIFFS (SYYML(1),AKVS(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
     *                ,DTS,LMIJ,SYYML(1),FLDUM)
        Call OVDIFFS (SXYML(1),AKVS(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2
...
#ifdef TRACERS_OCEAN
#ifdef OCN_GISS_TURB
c       akvc(1:lmij-1) = akvc3d(1:lmij-1,i,j)
c       akvc(lmij) = 0.
#endif
        do n=1,ntm
          txml(1:lmij) = txmo(i,j,1:lmij,n)
          tyml(1:lmij) = tymo(i,j,1:lmij,n)
          Call OVDIFFS ( TXML(1),AKVC(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2,
     *         DTS,LMIJ, TXML(1),FLDUM)
          Call OVDIFFS ( TYML(1),AKVC(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2,
     *         DTS,LMIJ, TYML(1),FLDUM)
          ...
          Call OVDIFFS (TXXML(1),AKVC(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2,
     *         DTS,LMIJ,TXXML(1),FLDUM)
          Call OVDIFFS (TYYML(1),AKVC(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2,
     *         DTS,LMIJ,TYYML(1),FLDUM)
          Call OVDIFFS (TXYML(1),AKVC(1),GHATDUM,DTP4DUM,DTBYDZ,BYDZ2,
...
C****
      ...
      function brylew_shape(z)
      ! shape function for B-L profile
      use constant, only : pi
      implicit none
      real*8 :: brylew_shape
      real*8 :: z ! meters
      brylew_shape = atan((z-2000d0)/200d0)/pi + 0.5d0
      end function brylew_shape
#endif
...
C**** End of outside loop over straits
  790 CONTINUE
      ...
      SUBROUTINE OVDIFF(U,K,GHAT,DTP4,DTBYDZ,BYDZ2,LMIJ,U0)
!@sum  OVDIFF Implicit vertical diff + non local transport for velocity
!@auth Gavin Schmidt
      USE OCEAN, only : LMO
      USE TRIDIAG_MOD, only : tridiag
      IMPLICIT NONE
      REAL*8, DIMENSION(LMO), INTENT(IN) :: U0,K,GHAT,DTBYDZ,BYDZ2,DTP4
...
c     USE DOMAIN_DECOMP_1D, only : dist_grid,getDomainBounds
...
c     call getDomainBounds(grid, J_STRT_HALO=J_0H, J_STOP_HALO=J_1H)
...
c    &     getDomainBounds,halo_update,south,north
     &     get,halo_update,south,north
      use oceanr_dim, only : grid=>ogrid
